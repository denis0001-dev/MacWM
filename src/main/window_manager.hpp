...
class WindowManager {
    ...
    private:
        struct ButtonWindows {
            Window close;
            Window minimize;
            Window maximize;
        };
        std::unordered_map<Window, ButtonWindows> button_windows_;

        // Invoked internally by Create().
        explicit WindowManager(Display* display);
        // Frames a top-level window.
        void frame(const Window w, const bool beforeWindowManager, bool wallpaper);
        ...
        void onUnknownEvent(const XEvent& event);
